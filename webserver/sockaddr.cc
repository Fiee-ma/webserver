...
static Logger::ptr g_logger = WEBSERVER_LOG_ROOT();
template<class T>
static T CreateMask(uint32_t bits) {
    return (1 << (sizeof(T) * 8 - bits)) -1;
}

// 统计二进制value里面有多少个１,太棒了
template<class T>
...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host,
    ...
    if(error) {
        WEBSERVER_LOG_ERROR(g_logger) << "Address::Lookup getaddrinfo(" << host << " "
...
std::shared_ptr<IPAddress> Address::LookupAnyIPAdress(const std::string &host, int family,
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t> > &result
    ...
    if(getifaddrs(&results) != 0) {
        WEBSERVER_LOG_ERROR(g_logger) << "Address::GetInterfaceAddress getifaddrs "
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    std::cout << "IPv4Address constrcut" << std::endl;
