...
void Logger::setFormatter(LogFormatter::ptr val) {
    ...
    for(auto &i : m_appenders) {
        MutexType::Lock ll(i->m_mutex); // 我觉得这里不用加
        if(!i->m_hasFormatter) {
...
void Logger::setFormatter(const std::string &str) {
    LogFormatter::ptr new_val(new LogFormatter(str));
    if(new_val->isError()){
        std::cout << "Logger setFormatter name=" << m_name
                  << "value=" << str << "invalid formatter"
                  << std::endl;
        return;
    }
    //m_formatter = new_val;
...
public:
    ...
    void format(std::ostream &os, Logger::ptr logger, LogLevel::Level level, LogEvent::ptr event) override {
        os << event->getLogger()->getName();  //用event提取出来的logger才是最原始的logger
    }
};
        ...
        while(n < m_pattern.size()){
            ...
            if(fmt_status == 0){
                ...
                if(m_pattern[n] == '}'){
                  ...
                  //std::cout<<"n = "<<fmt_begin << "fmt = "<<fmt<<std::endl;
                  fmt_status = 0;
                  ++n;
                  break;
                }
        ...
        if(n["appender"].IsDefined()) {
            for(size_t x = 0; x < n["appender"].size(); ++x) {
                ...
                if(type == "FileLogAppender") {
                    lad.type = 1;
                    if(!a["file"].IsDefined()) {
                        std::cout << "log config error: FileLogAppender file is null, " << a
                            << std::endl;
                        continue;
                    }
...
//                    lad.level = LogLevel::FromString(a["level"].IsDefined()
//                            ? a["level"].as<std::string>() : "");
...
server_name::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    server_name::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
        ...
        g_log_defines->addListener([](const std::set<LogDefine> &old_value,
            ...
            for(auto &i : new_value) {
               ...
               if(it == old_value.end()) {
                ...
                for(auto &a : i.appenders) {
                    server_name::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new server_name::FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        ap.reset(new server_name::StdoutLogAppender);
                    }

                    ap->setLevel(a.level);
                    if(!a.formatter.empty()) {
                        std::cout<< "a.formatter = " << a.formatter << std::endl;
                        server_name::LogFormatter::ptr fmt(new LogFormatter(a.formatter));
                        if(!fmt->isError()) {
                            ap->setFormatter(fmt);
                        } else {
                            std::cout<< "log.name=" << " appender type=" << a.type
